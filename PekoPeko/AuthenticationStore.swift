...
class AuthenticationStore {
    ...
    class func upPromoCode(authenticationRequest: AuthenticationRequest, completionHandler: @escaping (Bool, Error?) -> Void) {
        
        let parameters = authenticationRequest.toJSON()
        
        _ = Alamofire.request(Router.upPromoCode(parameters as [String : AnyObject])).responseBool({ (response) in
            if let error = response.result.error {
                completionHandler(false, error)
                return
            }
...
extension Alamofire.DataRequest {
    ...
    func responseBool(_ completionHandler: @escaping (DataResponse<Bool?>) -> Void) -> Self {
        let responseSerializer = DataResponseSerializer<Bool?> { request, response, data, error in
            guard error == nil else {
                return .failure(error!)
            }
