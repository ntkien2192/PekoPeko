...
class AuthenticationStore {
    ...
    class func loginWithFacebook(authenticationRequest: AuthenticationRequest, completionHandler: @escaping (AuthenticationResponse?, Error?) -> Void) {
        
        let parameters = authenticationRequest.toJSON()
        
        _ = Alamofire.request(AuthenticationRouter.loginWithFacebook(parameters as [String : AnyObject])).responseAuthentication({ (response) in
            if let error = response.result.error {
                completionHandler(nil, error)
                return
            }
...
extension Alamofire.DataRequest {
    ...
    func responseAuthentication(_ completionHandler: @escaping (DataResponse<AuthenticationResponse?>) -> Void) -> Self {
        let responseSerializer = DataResponseSerializer<AuthenticationResponse?> { request, response, data, error in
            guard error == nil else {
                return .failure(error!)
            }
